...
class ChatServer(threading.Thread):
	...
	def __init__(self, host, port):
		...
		self.host = host
		self.port = port
		self.connections = []  # collects all the incoming authorized connections
		self.unauthorizied_connections = [] ## collects all the incoming unauthorized connections(without nickanme)
		self.authorized_connections = {} # dictionary with nickname/address(client_socket)
		...
		for sock in self.connections:
			...
			if is_not_the_server and is_not_the_client_sending:
				try :
					...
					del self.authorized_connections[sock]
	...
	def _run(self):
		...
		"""
		print("Server is running on port: " + str(self.port))
		while self.running:
				...
				for sock in ready_to_read:
					...
					if sock == self.server_socket:
						try:
							...
							self.unauthorizied_connections.append(client_socket)
							print("Client " + str(client_address) + " connected")

							# Notifies all the connected clients a new one has entered
							#self._broadcast(client_socket, "\n[" + str(client_address) + "] entered the chat room\n" )
					# ...else is an incoming client socket connection
					else:
						try:
							data = self._receive(sock) # Gets the registered client message...
							if data:
								# ... and broadcasts it to all the connected clients or registers the user
								if sock in self.unauthorizied_connections:
									self.registration(sock, data)
								else:
									self._broadcast(sock, "\r" + '<' + self.authorized_connections[sock] + '> ' + data)
						except socket.error:
							# Broadcasts all the connected clients that a clients has left
							if sock not in self.unauthorizied_connections:
								self._broadcast(sock, "\nClient " + self.authorized_connections[sock] + " is offline\n")
							print("Client " + str(client_address) + " is offline")
	...
	def registration(self, socket, name):
		"""
		For user registration
		"""
		if name in self.authorized_connections.values():
			self._send(socket, "Username " + name + " already exists. Please try again")
		else:
			self.authorized_connections[socket] = name
			self.unauthorizied_connections.remove(socket)
